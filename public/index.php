...
<body>
        ...
        <div class="card">
            ...
            <?php if (empty($servers)): ?>
                <div class="empty-state">
                    ...
                    <p>Нажмите "Добавить сервер" чтобы начать</p>
                </div>
            <?php else: ?>
                <table>
                    <thead>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge badge-<?= $server['status'] === 'active' ? 'success' : ($server['status'] === 'error' ? 'danger' : 'warning') ?>">
                                    <?= htmlspecialchars($server['status']) ?>
                                </span>
                            </td>
                            <td><?= date('d.m.Y', strtotime($server['created_at'])) ?></td>
                            <td>
                                <a href="/server/<?= $server['id'] ?>/clients" class="btn btn-info btn-sm">👥 Клиенты</a>
                                <button class="btn btn-success btn-sm" onclick="testConnection(<?= $server['id'] ?>)">🔍 Тест</button>
                                <button class="btn btn-danger btn-sm" onclick="deleteServer(<?= $server['id'] ?>)">🗑</button>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
    ...
    <!-- Модальные окна (как в предыдущей версии) -->
    <div class="modal" id="addServerModal">
        <div class="modal-content">
            ...
            <form id="addServerForm">
                ...
                <div style="margin-bottom:16px;">
                    <label style="display:block;margin-bottom:4px;font-weight:500;">IP-адрес</label>
                    <input type="text" name="ip_address" required style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:6px;">
                </div>
                
                <div style="margin-bottom:16px;">
                    <label style="display:block;margin-bottom:4px;font-weight:500;">SSH порт</label>
                    <input type="number" name="ssh_port" value="22" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:6px;">
                </div>
                
                <div style="margin-bottom:16px;">
                    <label style="display:block;margin-bottom:4px;font-weight:500;">Пользователь</label>
                    <input type="text" name="ssh_username" value="root" style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:6px;">
                </div>
                
                <div style="margin-bottom:16px;">
                    <label style="display:block;margin-bottom:4px;font-weight:500;">Пароль root</label>
                    <input type="password" name="auth_value" required style="width:100%;padding:10px;border:1px solid #e2e8f0;border-radius:6px;">
                </div>
                
                <input type="hidden" name="auth_type" value="password">
                
                <div style="display:flex;gap:10px;justify-content:flex-end;">
                    <button type="button" onclick="closeAddServerModal()" style="padding:10px 20px;background:#e2e8f0;border:none;border-radius:8px;cursor:pointer;">Отмена</button>
    ...
    <script>
        ...
        async function deleteServer(id) {
            if (!confirm('Удалить сервер и всех клиентов?')) return;
            ...
            try {
                ...
                if (data.success) {
                    alert('✅ Сервер доступен!\n\n' +
                          'ОС: ' + data.os + '\n' +
                          'WireGuard: ' + (data.wireguard ? 'установлен' : 'не установлен') + '\n' +
                          'Аптайм: ' + data.uptime);
                } else {
